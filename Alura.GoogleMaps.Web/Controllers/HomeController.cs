...
namespace Alura.GoogleMaps.Web.Controllers
{
    public class HomeController : Controller
    {
        ...
        public async Task<JsonResult> Localizar(HomeViewModel model)
        {
            ...
            //Configurar os pontos atuais no mapa
            ...
            if(tipoAero == "")
            {
                filtro_builder = constructor.NearSphere(x => x.loc, location, distancia);
            }
            else
            {
                filtro_builder = constructor.NearSphere(x => x.loc, location, distancia) &
                                 constructor.Eq(x => x.type, tipoAero);
            }

            //captura a lista
            var aeroportos = await connection.Airports.Find(filtro_builder).ToListAsync();

            //Escreve os pontos
            foreach(var doc in aeroportos)
            {
